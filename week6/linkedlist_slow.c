...
void dumb_tree_put(struct dumb_tree *tree, int index, void *value) {
    ...
    current = tree->root;
    if (current == NULL) {
        tree->root = dumb_tree_node_new(index, value);
...
void *lookup(void *arg) {
    ...
    for (int i = args.index_start; i != args.index_end; i++) {
        ...
        if (value != args.array + i) {
            die("%d wrong value expecting %p got %p", i, args.array + i, value);
        }
    }
...
int main(int argc, char **argv) {
    ...
    for (int i = 0; i < num_thread; i++) {
        pthread_create(threads + i, NULL, update, thread_args + i);
    }
    for (int i = 0; i < num_thread; i++) {
        pthread_join(threads[i], NULL);
    }
    ...
    for (int i = 0; i < num_thread; i++) {
        pthread_create(threads + i, NULL, lookup, thread_args + i);
    }
    for (int i = 0; i < num_thread; i++) {
        pthread_join(threads[i], NULL);
    }
